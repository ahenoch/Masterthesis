...
if __name__ == "__main__":
    ...
    parser.add_argument('-i','--infile', type = str, required=True, help="path to input file (e. g. A.fasta)")
    parser.add_argument('-o','--outfolder', type = str, default = 'Results', help="path to input file (default: Results)")
    parser.add_argument('-s','--segments', nargs='+', type = int, choices=[1,2,3,4,5,6,7,8], default = [1,2,3,4,5,6,7,8], help="segments to run the pipeline on (default: 1 2 3 4 5 6 7 8)")
    parser.add_argument('-c','--custom_header', nargs='+', type = str, choices=['accession', 'strain', 'segment', 'protein', 'genus', 'subtype', 'date', 'host', 'curation', 'genome'], default = ['accession', 'strain', 'segment', 'protein', 'genus', 'subtype', 'date', 'host', 'curation', 'genome'], help="if using FASTA with a custom header, every part of it has to be declared (default: accession strain segment protein genus subtype date host curation genome)")
    parser.add_argument('-m','--metric', type = str, choices=['cosine','euclidean'], default = 'cosine', help="metric to use in the pipeline (default: cosine)")
    parser.add_argument('-mc','--min_clust', type = int, default = 2, help="min_cluster_size parameter for HDBSCAN (default: 2)")
    parser.add_argument('-ms','--min_sample', type = int, help="min_samples parameter for HDBSCAN (default: 1)")
    parser.add_argument('-n','--neighbors', type = int, default = 100, help="n_neighbors parameter for UMAP (default: 100)")
    parser.add_argument('-u','--umap', type = int, help="n_components parameter for UMAP (optional)")
    parser.add_argument('-p','--pca', type = int, help="n_components parameter for PCA (optional)")
    parser.add_argument('-k','--max_kneedle', type = int, default = 500, help="search area for Kneedle Algorithm (default: 500)")
    parser.add_argument('-r','--render', type = str, choices=['svg','pdf'], default = 'pdf', help="format of output graphics (default: pdf)")
    parser.add_argument('-e','--epsilon', type = str, choices=['dbcv','kneedle'], default = 'kneedle', help="method to use for epsilon exploration, hardcoded to kneedle, due to missing useful alternatives")
    parser.add_argument('-t','--threads', type = int, default = 12, help="number of threads to use for the pairwise cluster validation")
