...
\begin{figure}[!hbt]
    ...
    \end{subfigure}
    \caption[Chord calculation background]{\textbf{Chord calculation background.} The chord distance can be calculated, when having a similar radius $r$ for both vectors and the information of their angle $\Theta$ (left picture). Since this information is not always available or sometimes expensive to calculate for a high number of vectors, the euclidean distance can be used for similar results, with the precondition of a L2-norm of both vectors equal to $r$. L2-norm of $r$ is in this case, thus, similar to a radius of $r$, satisfying the condition. To approximate cosine distance, with chord distance, the same calculation of euclidean distance was used with L2-norm equal to $r = 1$. To satisfy the requirement that all vectors L2-norm had to equal one, L2-norm normalization was used on every vector scaling them to match the condition (right picture).}
...
Thus, in this project, chord distance can be calculated with the euclidean distance metric, posterior to the normalization with the L2-norm, which scales the vectors to the unit sphere, fulfilling the precondition for similarity by $r=1$ (\autoref{eq:chord2} and \autoref{subfig:L2}). 
...
The calculation of the chord distance, by L2-norm normalized vectors euclidean distance is, thereby, integrated into the mutual reachability distance calculation of \texttt{HDBSCAN}. The mutual reachability distance is the maximum of the described chord distance and the core distances of two vectors (\autoref{eq:reach}). It is the main calculation of \texttt{HDBSCAN} used for clustering. The core distance is the minimum radius necessary to include one other vector around a given vector \autocite{mcinnes_hdbscan_2017}. Threshold $\varepsilon$ is used on the mutual reachability distances between the vectors \autoref{fig:Hybrid}. Due to the parameter choice of $k=1$, the core distance can never be higher than the euclidean distance, making the mutual reachability distance always equal the chord distance calculated by euclidean distance. 

\begin{equation}\label{eq:reach}
    \begin{aligned}
        d_{\text{mreach}-k}(\mathbf{\hat{x}},\mathbf{\hat{y}}) &= \max \{ \text{core}_k(\mathbf{\hat{x}}), \text{core}_k(\mathbf{\hat{y}}), d_{\text{chord}}(\mathbf{\hat{x}},\mathbf{\hat{y}}) \}
    \end{aligned}
\end{equation}

The used chord distance, by euclidean distance calculation, is proportional with the initially intended to use cosine distance as shown in \autoref{eq:chord3}. Dividing the squared chord distance by two results in the cosine distance of the vectors \autoref{fig:Normalisation_Methods} \autocite{korenius_principal_2007}.
...
Second method for $\varepsilon$ exploration was performed using the Kneedle Algorithm on the linkage matrix created by the initially performed standard \texttt{HDBSCAN} clustering without hybrid setting (\autoref{fig:Clustering_Pipeline} \textsf{\textbf{F}} and \textsf{\textbf{I}}) \autocite{satopaa_finding_2011}. With increasing cluster number, the distance threshold $\varepsilon$ of hierarchical clustering decreases. This describes a decreasing curve of convex type, with distance threshold on the y- and cluster number on the x-axis. The knee is the number of clusters at the point in the polynomial representation of the curve with maximal acceleration. Polynomial representation was used to find the maximum acceleration of a more precise smoothed curve. Therefore the settings \texttt{curve='concave'}, \texttt{direction='increasing'} and \texttt{interp\_method='interp1d'} were used to find the optimal number of clusters. The optimal number of clusters was converted to the respective $\varepsilon$ threshold.
