...
\begin{figure}[!hbt]
    ...
    \end{subfigure}
    \caption[Cosine distance approximation]{\textbf{Cosine distance approximation.} The cosine distance of two vectors $\mathbf{x}$ and $\mathbf{y}$ is based on the cosine of the angle $\Theta$ between the vectors (left picture). Since this information is not always available or the alternative calculation using the vectors only is to expensive for a high number of vectors, the euclidean distance can be used for approximation, with the precondition of a L2-norm of both vectors equal to one. Therefore, to approximate cosine distance with euclidean distance, all vectors L2-norm had to be one. L2-norm normalization was used on every vector scaling them to the unit sphere to match this condition (right picture).}
...
Due to the L2-norm normalization according to \autoref{eq:norm2}, all vectors satisfy the condition in \autoref{eq:assumpt}.
...
The euclidean distance of the L2-norm normalized vectors, is in close relation to the cosine distance as proven in \autoref{eq:chord3}. Dividing the squared euclidean distance of the L2-norm normalized vectors by two, results in the cosine distance of the vectors \autoref{fig:Normalisation_Methods} \autocite{korenius_principal_2007}.
...
\begin{equation}\label{eq:chord3}
    ...
    \begin{aligned}  
        ...
        &= \Vert\mathbf{\hat{x}}\Vert_2^2 + \Vert\mathbf{\hat{y}}\Vert_2^2 - 2 \cos(\Theta)\\
        &= 2 (1 - \cos(\Theta))\\
