...
\begin{table}[!hbt]
    \centering
    \caption[\Acrshort{DBCV} based Clustering (\Acrshort{PCA})]{\textbf{\Acrshort{DBCV} based Clustering (\Acrshort{PCA}).} The clustering using the combination of \gls{PCA} and the \gls{DBCV} exploration, results in the given table. Listed is every used segment with the number of raw clusters and the final cluster number after hybrid clustering with the given value of $\varepsilon$. The number of raw clusters is based on directly on the $\varepsilon$ value. Mixed cluster numbers of H and N denotes the number of inconclusive clusters when comparing by the respective subtypes. The variance of the \gls{PCA} is calculated by the sum of the 30 components explained variance.}
...
\begin{table}[!hbt]
    \centering
    \caption[\Acrshort{DBCV} based Clustering (\Acrshort{UMAP})]{\textbf{\Acrshort{DBCV} based Clustering (\Acrshort{UMAP}).} The clustering using the combination of \gls{PCA}, \gls{UMAP} and the Kneedle Algorithm, results in the given table. Listed is every used segment with the number of raw clusters and the final cluster number after hybrid clustering with the given value of $\varepsilon$. The number of raw clusters is based on directly on the $\varepsilon$ value. Mixed cluster numbers of H and N denotes the number of inconclusive clusters when comparing by the respective subtypes. The variance of the \gls{PCA} is calculated by the sum of the 100 components explained variance.}
